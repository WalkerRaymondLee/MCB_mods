...
!     lchnk, ncol )
    MCB_seeding_mask, &
    lchnk, ncol, MCB_seeding_mask_out )
!++ MCB revised     
!-- MCB
...
!++ MCB 
!++ MCB revised
  real(r8), intent(in) :: MCB_seeding_mask(mgncol)
  real(r8) :: MCB_seeding_mask_out(mgncol)
...
!  call get_MCB_mask(lchnk, ncol, MCB_seeding_mask)
  MCB_seeding_mask_out = MCB_seeding_mask(:mgncol)
!-- MCB revised
!-- MCB
...
!++ MCB
  ! change nctend to make ncic = 400 #/cm3
  do i=1,mgncol                                                                 ! for each microphysics column
    if(MCB_seeding_mask(i)>=0.95_r8) then                                       ! if this is part of the MCB mask
      do k=1,nlev                                                               ! for each vertical level
        if(p(i,k)>=85000._r8) then                                              ! if pressure >= 850 hPa
            nctend(i,k) = (4.00e8_r8/rho(i,k)*lcldm(i,k)-ncn(i,k))/deltatin     ! change CDNC to 375 #/cm3
         end if
      end do
     end if
  end do
