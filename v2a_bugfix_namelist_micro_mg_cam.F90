...
!++ MCB namelist
  call mpi_bcast(MCB_seeding_amt, 1, mpi_real8, mstrid, mpicom, ierr)
  if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: MCB_seeding_amt")
!-- MCB namelist
...
!++ MCB namelist
     write(iulog,*) 'MCB seeding amount'
     write(iulog,*) '  MCB_seeding_amt = ', MCB_seeding_amt
!-- MCB namelist
  end if
...
subroutine micro_mg_cam_tend(state, ptend, dtime, pbuf, MCB_seeding_mask)
!-- MCB revised
   ...
   !++ MCB revised
   real(r8), intent(in) :: MCB_seeding_mask(pcols)
   !-- MCB revised
   ...
   call micro_mg_cam_tend_pack(state, ptend, dtime, pbuf, mgncol, mgcols, nlev, MCB_seeding_mask)
   !-- MCB revised
...
subroutine micro_mg_cam_tend_pack(state, ptend, dtime, pbuf, mgncol, mgcols, nlev, MCB_seeding_mask)
!-- MCB revised
   ...
   !++ MCB revised
   real(r8) :: packed_MCB_seeding_mask(mgncol) ! subset of MCB_seeding_mask(pcols), columns with active microphysics only
   !-- MCB revised
...
!++ MCB namelist
!   MCB_seeding_amt_ID = MCB_seeding_amt + 1 ! if namelist = 0 (off), use mask 1 (empty); if namelist = 1 (on), use mask 2
!-- MCB namelist
  ...
  if (.not. do_cldice) then
   ...
   !++ MCB revised
   packed_MCB_seeding_mask = packer%pack(MCB_seeding_mask) ! obtain subset of MCB_seeding_mask with active microphysics only
   !-- MCB revised
